...
{
    class QazanfarsProblem
    {
        static readonly double parameter = 0.01;
        static Vector f(double t, Vector y, double parameter)
        {
            double y0 = y[1];
            double y1 = y[3] - y[0] / (1 + parameter * t * t);
            double y2 = -y[3] / (1 + parameter * t * t);
            double y3 = -y[2];
        ...
        static double a = Math.Pow(Math.E, -Math.PI / 2);
        static double b = Math.Pow(Math.E, Math.PI / 2);
        static readonly Vector analyticalSolutionForInitialParameter =
            new Vector(((-Math.PI / 2) * (a - b) - Math.PI) / (a + b), 2 * Math.PI / (a + b));
        static readonly sbyte requiredNumOfPoints = 1;
